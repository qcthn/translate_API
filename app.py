...
# Thư viện cho Word, PowerPoint, PDF, Excel
...
def translate_text_with_chatgpt(original_text, api_key, global_dict=None):
    ...
    system_prompt = (
        "Bạn là một trợ lý AI dịch thuật, với kinh nghiệm 10 năm trong việc dịch các văn bản từ ngôn ngữ tiếng Anh sang tiếng Việt. Hãy dịch các văn bản sau từ tiếng Anh sang tiếng Việt, "
...
########################################################################
# 4) XỬ LÝ ĐỊNH DẠNG DOCX / PPTX / PDF / EXCEL
########################################################################
...
def translate_excel(excel_file, api_key, specialized_dict):
    ...
    for sheet_name in excel_data.sheet_names:
        ...
        for col in df.columns:
            if df[col].dtype == object:
                df[col] = df[col].apply(lambda x: translate_text_with_chatgpt(str(x), api_key, specialized_dict) if pd.notnull(x) else x)
...
st.title("Tự động dịch tài liệu (PPTX, DOCX, PDF, EXCEL) + Giữ định dạng")
...
uploaded_file = st.file_uploader("Tải lên file cần dịch (PPTX, DOCX, PDF, EXCEL)", type=["pptx", "docx", "pdf", "xlsx"])
...
if uploaded_file and api_key:
    ...
    if st.button("Bắt đầu dịch"):
        if ext == "pptx":
            ...
            output = translate_excel(uploaded_file, api_key, specialized_dict)
            st.download_button("Tải về file Excel đã dịch", output, "translated.xlsx")
            st.success("Hoàn thành dịch Excel!")
            st.stop()
        else:
    ...
    st.success("Hoàn thành dịch tài liệu!")
